jQuery(document).ready(function($) {
    'use strict';
    var this_obj = vw_gardening_landscaping_plugin_activate_plugin;

    $(document).on('click', '.vw-gardening-landscaping-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message vw-gardening-landscaping-plugin-install');
                button.addClass('button-primary vw-gardening-landscaping-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.vw-gardening-landscaping-recommended-plugins .vw-gardening-landscaping-activation-note').css('display','block');
    ...
    $(document).on('click', '.vw-gardening-landscaping-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .vw-gardening-landscaping-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .vw-gardening-landscaping-recommended-plugins').css('display','none');
    });
